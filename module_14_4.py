...
initiate_db('products.db')# инициализируем БД и создаем (если нет) таблицу продуктов
#insert_db('products.db') заполняем таблицу 4-мя продуктами
prod_list = get_all_products('products.db')# загружаем в переменную данные из таблицы БД (каждая строка - кортеж в списке, поле - элемент кортежа)
